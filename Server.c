...
bool Register(cJSON* cjson)
{
        ...
        sprintf(sql,"%s'%s'","select id,username,password from user where username=",cJSON_GetObjectItem(cjson,"username")->valuestring);//生成sql语句
        ...
        {
		sprintf(sql,"%s","select count(id) from user");//生成sql语句
		mysql_query(&mysql,sql);//执行sql语句
        	result=mysql_store_result(&mysql);//sql查询结果
		...
		sprintf(sql,"%s(%d,'%s','%s')","insert into user values",i,cJSON_GetObjectItem(cjson,"username")->valuestring,cJSON_GetObjectItem(cjson,"password")->valuestring);//生成sql语句
		mysql_query(&mysql,sql);//执行sql语句
	...
	printf("查询完毕\n");
    	...
    	if(row=mysql_fetch_row(result))//如果查到东西了
    	{
			...
			printf("%d\n",i);
    			...
    			strcpy(online[i].username,cJSON_GetObjectItem(cjson,"username")->valuestring);
