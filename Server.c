...
bool searchPerson(cJSON* cjson)
{
	...
	if(row = mysql_fetch_row(result))//如果查到东西了
	{
		...
		cJSON_AddNumberToObject(back,"state",1);
        	...
        	cJSON_AddNumberToObject(back,"usergender",atoi(row[5]));
...
bool Update_User_Info(cJSON* msg)
{
	...
	//存入数据库
	char sqlstring[1024];
	sprintf(sqlstring, "update user set usernickname='%s', usergender=%d, userintro='%s', user_selfiefilename='%s', userbirthday='%s', userage=%d  where id=%d", usernickname, usergender, userintro, user_selfiefilename, userbirthday, userage, id);//对数据库中的个人信息进行更新
	printf("sql:%s\n", sqlstring);
...
bool Register(cJSON* cjson)
{
	printf("注册\n");
		...
		sprintf(sql,"insert into user values(%d,'%s','%s','%s','%s','%s','%s','%s',%d,%d,'%s')",i,cJSON_GetObjectItem(cjson,"username")->valuestring,cJSON_GetObjectItem(cjson,"password")->valuestring," "," "," "," "," ",0,0," ");//向user表添加一条记录，包括新注册用户的帐号密码以及为他分配的id
		mysql_query(&mysql,sql);//执行sql语句
...
bool addFriend(cJSON* cjson)
{
	...
	{
		sprintf(sql,"insert into conversation (id1,id2,cindex) values(%d,%d,-1)",cJSON_GetObjectItem(cjson,"recvid")->valueint,cJSON_GetObjectItem(cjson,"sendid")->valueint);//存储好友申请
                mysql_query(&mysql,sql);//执行sql语句
...
bool createGroup(cJSON* cjson)//创建群聊
{
	...
	mysql_query(&mysql,sql);//执行sql语句
    	...
    	sprintf(sql,"insert into chatgroup (cnumber, groupname, ownerID) values (%d,'%s',%d)",-newnum,cJSON_GetObjectItem(cjson,"groupname")->valuestring,cJSON_GetObjectItem(cjson,"ownerID")->valueint);
    	mysql_query(&mysql,sql);//执行sql语句
